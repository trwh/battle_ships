...
describe Player do
  ...
  it 'responds to fire with 2 arguments' do
    expect(Player).to respond_to(:new).with(2).argument
  end

  it 'knows where the player has fired on the opponent\'s board' do
    player1.fire(board_hit, [0,0])
    player1.fire(board_hit, [0,1])
    player1.fire(board_miss, [0,2])
